...
def editar_trabajo_extra(request, pk):
    trabajoExtra = TrabajoExtraordinario.objects.get(id=pk)
    form = TrabajoExtraordinarioForm(request.POST or None, instance=trabajoExtra)
    if form.is_valid():
        form.save()
        return redirect('/Trabajo_extraordinario/')
    cal = TrabajoExtraordinario.objects.all()
...
def reporte(request):
    ...
    return render(request, 'Reporte_Vacaciones.html', request_report(fecha_inic, fecha_fin))
