...
def reporte(request):
    ...
    return render(request, 'Reporte_Ausentismo.html', request_report(fecha_inic, fecha_fin))
...
def gestionar_tarjeta_cnc(request):
    list_trab = Trabajador.objects.all()
    form = TarjetaCNCForm(request.POST or None)
    context = {'list_trab': list_trab, 'form': form}
    return render(request, 'Gestionar_TarjetaCNC.html', context)


def listar_tarjeta_cnc(request, pk):
    trabajador = Trabajador.objects.get(id=pk)
    list_entradas = TarjetaCNC.objects.filter(codigo_trab=pk)
    form = TarjetaCNCForm(request.POST or None)
    context = {'list_entradas': list_entradas, 'form': form, 'trab': trabajador}
    return render(request, 'Listar_TarjetaCNC.html', context)


def adicionar_tarjeta_cnc(request):
    lista_entradas = []
    form = TarjetaCNCForm(request.POST or None)
    if form.is_valid():
        ...
        if TarjetaCNC.objects.filter(mes=form.cleaned_data['mes'], anno=form.cleaned_data['anno'],
            ...
            form.save()
            return redirect('/Listar_RegistroTarjetaCNC/' + tarjeta.codigo_trab.pk.__str__() + '/')
    if request.environ['HTTP_REFERER'] == 'http://' + request.environ['HTTP_HOST'] + '/RegistroTarjetaCNC/' or \
        request.environ['HTTP_REFERER'] == 'http://' + request.environ[
                'HTTP_HOST'] + '/Adicionar_RegistroTarjetaCNC/':
        cal = Trabajador.objects.all()
        context = {'list_trab': cal, 'form': form}
        return render(request, 'Gestionar_TarjetaCNC.html', context)
    else:
        return listar_tarjeta_cnc(request, form.cleaned_data['codigo_trab'].pk)


def editar_tarjeta_cnc(request, idtrab, pk):
    lista_entradas = []
    ...
    if form.is_valid():
        ...
        if bandera:
            ...
            form.save()
            return redirect('/Listar_RegistroTarjetaCNC/' + idtrab.__str__() + '/')
    trab = Trabajador.objects.get(id=int(idtrab))
...
def eliminar_tarjeta_cnc(request, idtrab, pk):
    if request.method == 'POST':
        try:
            tarjeta = TarjetaCNC.objects.get(id=pk)
        except ObjectDoesNotExist:
        ...
        try:
            ...
            context = {'error': 'El objeto que intenta eliminar no existe.', 'idtrab': idtrab}
            return render(request, 'Eliminar_TarjetaCNC.html', context)
